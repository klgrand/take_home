...
export const mergedCartItem = (cartItems: CartItemProps[]): CartItemProps[] => {
  ...
  console.log('mergedCartItem .. ' , cartItems)
...
export const checkAvailableCoupon = (code: string): boolean => {
  const availableCoupon = CAMPAIGN_LIST.flatMap(item => item.coupon).find(item => item.coupon === code)
...
export const calculateFixedAmount = (cart: CartProps): CartProps => {
  ...
  const discountCoupon = _.find(fixedAmountPromo?.coupon, { coupon: fixedAmountPromo?.code })
  const discount = _.get(discountCoupon, 'discount', 0)
  if(cart.cart_total - discount < 0) throw new Error('Cannot add this campaign because it over discount.')
...
export const calculatePercentage = (cart: CartProps): CartProps => {
  ...
  if(cart_total - discount < 0) throw new Error("Cannot add this campaign because it over discount.")
  _.set(cart, 'discount', cart.discount + discount)
...
export const calculatePercentageByCat = (cart: CartProps): CartProps => {
  ...
  const promo = _.find(promotion, { _key: "percentage_discount_by_item_category" })
  const discountCoupon = _.find(promo?.coupon, { coupon: promo?.code })
  const percentDiscount = _.get(discountCoupon, 'discount', 0)
...
export const calculatePromotions = (cart: CartProps): CartProps => {
  ...
  for(const promotion of promotions) {
    ...
    if(promotion.campaign_category === "on_top") {
      if(promotion._key === 'percentage_discount_by_item_category') calculatePercentageByCat(cart)
...
export const addCouponToCart = (cart: CartProps, coupon: string) => {
  let campaign = _.cloneDeep(CAMPAIGN_LIST).find(item => item.coupon.find(item => item.coupon === coupon))
  if(campaign) {
    _.set(campaign, 'code', coupon)
    if(_.isEmpty(cart.promotion)) {
      cart["promotion"].push(campaign)
    } else {
      const foundSameCategory = _.find(cart["promotion"], { campaign_category: campaign.campaign_category })
      if(!foundSameCategory) {
        cart["promotion"].push(campaign)
      } else {
        for(let i = 0; i < cart["promotion"].length; i++) {
          if(cart["promotion"][i].campaign_category === campaign.campaign_category) {
